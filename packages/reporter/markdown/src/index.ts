...
export class MarkdownReporter implements ReportGenerator {
    async generateReport(stats: DailyStats[]): Promise<string> {
        ...
        const totalCommits = stats.reduce((sum, day) => sum + day.commits.length, 0);
        const totalTimeSpent = stats.reduce((sum, day) => sum + day.totalTimeSpent, 0);
        ...
        const sortedStats = [...stats].sort((a, b) => b.date.getTime() - a.date.getTime());
