...
describe("TimeSheet", () => {
	...
	describe("generateReport", () => {
		...
		it("should group commits by day", async () => {
			...
			const reporterCalls = (mockReporter.generateReport as any).mock.calls;
			expect(reporterCalls).toHaveLength(1);

			const dailyStats = reporterCalls[0][0] as DailyStats[];
			...
			expect(firstDay.firstCommitTime).toEqual(
			...
			expect(secondDay.firstCommitTime).toEqual(
				new Date("2024-01-11T10:00:00Z"),
			);
			expect(secondDay.lastCommitTime).toEqual(
				new Date("2024-01-11T10:00:00Z"),
			);
	...
	describe("generateReportByWindow", () => {
		...
		it("should calculate correct time range for days", async () => {
			...
			const vcsCall = (mockVcs.getCommits as any).mock.calls[0][0] as TimeRange;
